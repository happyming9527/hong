...
export default class UserFeeds extends React.Component {
  ...
  fetchData() {
    ST.httpPost(
      `/api/reported_feeds/list`, this.state.searchParams)
      .then(result=> {
  ...
  research() {
    ST.historyReload('/backend/reported_feeds?'+queryString.stringify(this.state.searchParams))
    this.fetchData()
  }
  ...
  render() {
    ...
    return (
      <SingleContainer breadcrumb={breadcrumb} header={header}>
        <List
          ref={i=>this.list=i}
